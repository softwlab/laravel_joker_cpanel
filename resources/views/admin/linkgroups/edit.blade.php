...
@section('title', 'Editar Grupo Organizado')

@section('content')
<div class="container-fluid">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Editar Grupo Organizado: {{ $group->name }}</h1>
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-body">
                    <form action="{{ route('admin.linkgroups.update', $group->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('usuario_id') is-invalid @enderror" id="usuario_id" name="usuario_id" required>
                                ...
                                <option value="{{ $usuario->id }}" {{ old('usuario_id', $group->usuario_id) == $usuario->id ? 'selected' : '' }}>{{ $usuario->nome }} ({{ $usuario->email }})</option>
                                @endforeach
                            </select>
                            @error('usuario_id')
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="form-check mb-3">
                            <input class="form-check-input" type="checkbox" id="active" name="active" {{ old('active', $group->active) ? 'checked' : '' }}>
                                ...
                                <div class="card-body" style="max-height: 300px; overflow-y: auto;">
                                    @if($banks->count() > 0)
                                        ...
                                        @foreach($banks as $bank)
                                            <div class="form-check">
                                                <input class="form-check-input bank-checkbox" type="checkbox" id="bank-{{ $bank->id }}" name="banks[]" value="{{ $bank->id }}" {{ in_array($bank->id, old('banks', $selectedBanks)) ? 'checked' : '' }}>
        ...
        <div class="col-md-4">
            ...
            <div class="card shadow-sm">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.linkgroups.destroy', $group->id) }}" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir este grupo organizado?')">
                        @csrf
                        @method('DELETE')
